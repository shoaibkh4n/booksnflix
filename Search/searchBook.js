...
const searchValue = requiredSearchValue.get('searchbook');
...
.then(function searchData(snapShot){
  ...
  searchValueArray.map(value=>{
    ...
    if(value != '' && value != 'the' && value != 'a' && value != 'an'  ){
      if(dataValues.movieName.toString().toLowerCase().includes(value)  || dataValues.genre.toString().toLowerCase().includes(value) || dataValues.platform.toString().toLowerCase().includes(value) || dataValues.yearRelease.toString().includes(value) || dataValues.type.toString().toLowerCase().includes(value) || dataValues.bookAuthor.toString().toLowerCase().includes(value) || dataValues.relatedBookName.toString().toLowerCase().includes(value) || dataValues.typeBook.toString().toLowerCase().includes(value) || dataValues.yearOfPublication.toString().includes(value)){
...
console.log(finalArrayUniqueValues);
  ...
  function searchLength(){
    if(results == 0){
     noResult.innerHTML = `
     <img src="../images/notfound.svg" alt="No book found image">
     `;
    }else if(results == 1){
      numberOfResults.innerHTML = `(Only 1 book found)`;
    }else {
      numberOfResults.innerHTML = `(About ${results} books found)`
    }
  }
  searchLength();
  ...
  function loopDataAndDisplayContent(displayedContent) {
    displayedContent.forEach(function(Snapshot) {
      ...
      const bookCardImage = dataObject.bookCardImage;
      const movieDescriptions = dataObject.movieDescription;
      const bookName = dataObject.relatedBookName;
